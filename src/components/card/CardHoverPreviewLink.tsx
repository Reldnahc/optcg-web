...
export function CardHoverPreviewLink({
  ...
  const handlePreviewIntent = () => {
    ...
    setPreviewStyle((current) => current.visibility === "hidden"
      ? current
  ...
  useLayoutEffect(() => {
    ...
    const updatePreviewPosition = () => {
      ...
      let nextLeft = wrapperRect.left + (wrapperRect.width / 2) - (previewRect.width / 2);
      const minLeft = GAP;
      const maxLeft = Math.max(GAP, viewportWidth - previewRect.width - GAP);
      nextLeft = Math.min(Math.max(nextLeft, minLeft), maxLeft);

      const rawTop = nextVertical === "top"
        ? wrapperRect.top - previewRect.height - GAP
        : wrapperRect.bottom + GAP;
      const maxTop = Math.max(GAP, viewportHeight - previewRect.height - GAP);
      const nextTop = Math.min(Math.max(rawTop, GAP), maxTop);
  ...
  }, [isOpen, previewPosition, preview?.card_number, effectText, triggerText, thumbnailUrl]);
    ...
    >
      ...
      {isOpen && typeof document !== "undefined"
        ? createPortal(
            <span
              ref={previewRef}
              className="pointer-events-none fixed z-[60] hidden w-[34rem] md:block"
              style={previewStyle}
            >
